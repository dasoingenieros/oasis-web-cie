...
function SupplyResultBanner({ result }: { result: any }) {
  ...
  return (
    <div className={`rounded-lg border p-4 ${ok ? 'border-emerald-200 bg-emerald-50' : 'border-amber-200 bg-amber-50'}`}>
      <div className="flex items-center gap-2 mb-3">
        <Zap className={`h-4 w-4 ${ok ? 'text-emerald-600' : 'text-amber-600'}`} />
        <span className={`text-sm font-semibold ${ok ? 'text-emerald-700' : 'text-amber-700'}`}>
          Suministro calculado {ok ? '— cumple REBT' : '— revisar advertencias'}
        </span>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-2 text-sm">
        <div>
          <span className="text-slate-500 text-xs">Potencia diseño</span>
          <p className="font-medium">{(result.designPowerW / 1000).toFixed(2)} kW</p>
        </div>
        {result.electrificationGrade && (
          <div>
            <span className="text-slate-500 text-xs">Electrificación</span>
            <p className="font-medium">{result.electrificationGrade === 'basic' ? 'Básica' : 'Elevada'}</p>
          </div>
        )}
        <div>
          <span className="text-slate-500 text-xs">IGA</span>
          <p className="font-bold text-blue-700">{result.iga.ratingA} A</p>
        </div>
        <div>
          <span className="text-slate-500 text-xs">Sección DI</span>
          <p className="font-bold text-blue-700">{result.di.sectionMm2} mm²</p>
        </div>
        <div>
          <span className="text-slate-500 text-xs">CdT DI</span>
        ...
        <div>
          <span className="text-slate-500 text-xs">Conductor PE</span>
          <p className="font-medium">{result.protectionConductorMm2} mm²</p>
        </div>
        <div>
          <span className="text-slate-500 text-xs">Diferenciales</span>
          <p className="font-medium">{result.differentials.length} × {result.differentials[0]?.sensitivitityMa ?? 30} mA</p>
        </div>
        <div>
          <span className="text-slate-500 text-xs">I nominal</span>
...
function ProyectoProximamente() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <h2 className="text-lg font-semibold text-slate-700">Proyecto Técnico — Próximamente</h2>
  ...
  if (error || !installation) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <p className="text-sm text-red-500">{error || 'Instalación no encontrada'}</p>
        ...
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.badge && !tab.disabled && (
              <span className="ml-1 rounded-full bg-slate-100 px-1.5 py-0.5 text-2xs font-medium text-slate-600">
                {tab.badge}
              </span>
            )}
          </button>
